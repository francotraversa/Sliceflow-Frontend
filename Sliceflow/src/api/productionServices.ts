...
export const productionService = {
  ...
  getHistoricalOrders: async (filters?: {
    status?: string;
    from_date?: string;
    to_date?: string;
    id?: string;
    ...
    if (!res.ok) throw new Error('Error al obtener el historial de órdenes');
    return await res.json();
  },

  getOrderById: async (id: string | number): Promise<ProductionOrder> => {
    const res = await fetch(`${API_BASE_URL}/hornero/authed/orders/list?id=${id}`, {
  ...
  createOrder: async (orderData: CreateOrderDTO) => {
    const token = localStorage.getItem('token');
    const res = await fetch(`${API_BASE_URL}/hornero/authed/orders/order`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  updateMaterial: async (id: number, data: any) => {
    const token = localStorage.getItem('token');
    const res = await fetch(`${API_BASE_URL}/hornero/authed/materials/updmat/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      body: JSON.stringify(data)
    });
  ...
  deleteMaterial: async (id: number) => {
    const token = localStorage.getItem('token');
    await fetch(`${API_BASE_URL}/hornero/authed/materials/delmat/${id}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token}` }
    });
  },
